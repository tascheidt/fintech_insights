...
- **The transient-retry budget rides out the *whole* Supabase recovery, not just the start of it.** `withTransientRetry` (ingest) and `withRetry` (mark-failed) both run **8 attempts / ~95s of backoff** (1,2,4,8,16,32,32s) and are kept in lock-step so a long outage that fails the ingest can still mark the task `failed`. The budget was widened from 6 attempts/~63s after the June 17 2026 TD ingest: a clean 1,574-job fetch+enrich died on a ~55s+ origin outage (`520/521` → `PGRST002`) that exhausted the six attempts in ~49s. If a *single* run keeps blowing 8 attempts, that's a sustained Supabase incident, not something to paper over with a bigger budget.
