...
- **`structure.ts`** — `extractJobStructure` (Flash, `thinkingBudget: 0` — reasoning tokens are pure waste on mechanical JSON extraction; ~72% per-call cost cut). Pulls structured fields out of the raw description. Called from `web/lib/jobs/processor.ts` via `extractAndUpdateStructure`, gated by a **normalized** `description_hash` (`normalizeDescriptionForHash` strips volatile boilerplate so unchanged postings skip the call). Evaluated against Flash-Lite in 2026-05 and **kept on Flash** (failed the ≥95% L1 gate). **It is not the only writer of `salary_min/max/currency`:** a scraper whose ATS publishes structured compensation (Ashby — see `web/lib/scrapers/ashby.ts`) writes the range at ingest and passes `preserveScrapedSalary`, which makes `extractAndUpdateStructure` skip those three columns. Published beats inferred, and on an Ashby board the range isn't in the description at all, so the extractor's null would otherwise erase it. See `docs/INGESTION_PIPELINE.md` § Salary provenance. Carries an **eval-only seam**: when handed an eval model id (e.g. `glm-5.2`) the generate step routes to the OpenAI-compatible provider; a Gemini id (the production default) runs the existing Gemini path unchanged. See [`docs/OPEN_MODEL_EVALUATION.md`](../../../docs/OPEN_MODEL_EVALUATION.md).
...
- **`search-report.ts`** — `generateSearchReportSynthesis` (Flash, ungrounded). Writes the block at the top of a user-generated shareable report: what kind of roles a /jobs search returned, how they cluster, and the requirements that recur. Called on demand from `POST /api/reports`, never from ingestion. Samples up to 40 rows round-robin across companies so one high-volume employer can't skew the clusters. **Failure is non-fatal by contract** — it returns `null` and the report renders table-only, because the output is behind a public share link that must not 500. User-facing → voice directive applies. See [`docs/SHARED_REPORTS.md`](../../../docs/SHARED_REPORTS.md).
