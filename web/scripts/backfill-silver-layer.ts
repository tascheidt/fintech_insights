...
async function main() {
  ...
  for (const job of jobs) {
    ...
    try {
      // Use the existing extractJobStructure function
      const structure = await extractJobStructure(job.title, job.description_text || "");
      ...
      const { error: updateError } = await supabase
        ...
        .update({
          ...
          keywords: [], // Empty for now as per processor.ts
