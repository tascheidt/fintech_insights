/**
 * TEMPLATE: Admin Feedback Review Table
 *
 * Copy this file into your app and customize:
 * - UI imports: Replace @/components/ui/* with your app's component library
 * - ADMIN_API_PATH: Must match your feedbackConfig.adminApiBasePath
 * - GitHub features: Set ENABLE_GITHUB and ENABLE_CODEGEN based on your config
 */
"use client";
...
// TODO: Must match your feedbackConfig.adminApiBasePath
const ADMIN_API_PATH = "/api/admin/feedback";

// TODO: Set based on whether you configured github in your feedbackConfig
const ENABLE_GITHUB = true;
const ENABLE_CODEGEN = true;
...
export function FeedbackReviewTable() {
  ...
  const fetchFeedback = useCallback(async () => {
    try {
      const url = filter === "all" ? ADMIN_API_PATH : `${ADMIN_API_PATH}?status=${filter}`;
  ...
  async function handleAction(id: string, decision: "accepted" | "declined") {
    ...
    try {
      ...
      const res = await fetch(ADMIN_API_PATH, {
        method: "PATCH",
  ...
  async function handleGenerateCode(id: string) {
    ...
    try {
      const res = await fetch(`${ADMIN_API_PATH}/${id}/generate-code`, { method: "POST" });
      if (res.ok) {
                ...
                <div key={item.id} className="py-3">
                  ...
                  {isExpanded && (
                    <div className="mt-3 ml-7 space-y-3">
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-1">
                              {ENABLE_GITHUB && item.github_issue_url ? (
                                <a
                      ...
                      {ENABLE_GITHUB && item.status === "accepted" && (
