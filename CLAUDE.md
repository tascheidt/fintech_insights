...
**Two Vercel crons (collect, report) + five GH Actions crons (company-insights, editorial, gemini-cost-alarm, embeddings-backfill, scraper-drift-check).** Hard cap: never more than two Vercel crons. Most GH Actions crons `curl` back into the deployed app with `Bearer ${CRON_SECRET}`; the editorial, embeddings-backfill, and scraper-drift-check crons run their script directly in the runner.
...
- GitHub Actions weekly Mon 12:00 UTC: `scraper-drift-check.yml` — runs `web/scripts/scraper-drift-check.ts` to compare every company's stored ATS config against its live careers surface (dormant company with a live board / careers_url resolving to a different ATS / dead configured endpoint) and email findings to admins via Resend. No Gemini calls. Born from the Koho incident (July 2026); rules in `web/lib/scrapers/drift.ts`, details in `docs/OBSERVABILITY.md`.
...
**Incumbent-tracking flag.** `collect`, `report` (Incumbent Watch), the `editorial` cron, and `backfill-incumbents.ts` all honor the `incumbent_tracking_enabled` flag (see §7). When it's off (the default), `collect` excludes `tier='incumbent'` companies at company-selection time — so incumbents are never scraped, extracted, or analyzed — and the digest's Incumbent Watch block is skipped. The flag is read via `getIncumbentTrackingEnabled` in `web/lib/settings/incumbent-tracking.ts`. **Exception:** an incumbent-tier parent whose scrape also feeds an active `tier='fintech'` sub-brand (e.g. Simplii piggy-backs on CIBC's Workday tenant via `companySlugOverride` — see `workday-cibc.ts`) is still included in `collect`'s company list even with the flag off, or the sub-brand would silently stop updating too (July 2026 incident). The heavy scraper pins that run to sub-brand-only mode across scrape and ingest. For CIBC, Workday's `searchText: "Simplii"` narrows the source corpus before detail enrichment; the existing classifier remains authoritative, and `runIngestStage` (`web/lib/jobs/processor.ts`) keeps only resolved Simplii rows (including child-scoped closure detection). CIBC's own postings are therefore neither enriched, ingested, nor analyzed. Re-enabling incumbent tracking restores the full CIBC corpus.
...
- **Feedback review axes:** like the two `is_active` axes, `feedback_submissions` has two independent state columns that MUST NOT be conflated — `triage_decision` (`yes`/`maybe`/`no`, what the AI concluded) vs `review_state` (`needs_review`/`approved`/`rejected`/`shipped`, what a human decided). `status` is a **legacy** mirror of the AI verdict, kept for compatibility only. The admin queue keys off `review_state` and defaults to `needs_review`; triage never writes `review_state`, so an AI verdict routes a submission into the queue but never resolves it, and every state is reversible (`needs_review` is a legal PATCH target). Conflating them is what made an AI decline final in the UI and silently buried two real bug reports — see [`docs/FEEDBACK_PIPELINE.md`](./docs/FEEDBACK_PIPELINE.md).
- **Feedback column ownership:** `feedback_submissions` is written by three principals — the submitting user (`user_id`, `type`, `title`, `description`, `page_url`), the triage engine (`status`, `triage_*`, `generated_issue`), and admins (`admin_*`, `reviewed_*`, `github_*`). The split is enforced by **Postgres column-level grants** (migration `20260731093000`), because RLS scopes rows and cannot scope columns — and the anon key ships in the browser, so any signed-in user can reach PostgREST directly and skip the route handler. Never `GRANT INSERT` on `status` or any `triage_*` column, and never widen these back to a table-wide grant to clear a permission error: that error means code is writing a column it doesn't own. Details and the pre-fix exploit in [`docs/FEEDBACK_PIPELINE.md`](./docs/FEEDBACK_PIPELINE.md).
...
- Granting `authenticated` INSERT or UPDATE on `feedback_submissions.status` / `triage_*` / `generated_issue` (or restoring a table-wide grant). See §7 "Feedback column ownership".
- Letting AI triage write `feedback_submissions.review_state`, or filtering the admin review queue by `status`. The AI verdict and the human decision are separate axes — see §7 "Feedback review axes".
- Rendering `triage_reasoning` / `triage_error` on a user-facing surface. They're internal classification output written for admins, and on failure they hold raw model error text.
...
- [`docs/CRON_TOPOLOGY.md`](./docs/CRON_TOPOLOGY.md), [`docs/AI_HYGIENE.md`](./docs/AI_HYGIENE.md), [`docs/INGESTION_PIPELINE.md`](./docs/INGESTION_PIPELINE.md), [`docs/OBSERVABILITY.md`](./docs/OBSERVABILITY.md), [`docs/FEEDBACK_PIPELINE.md`](./docs/FEEDBACK_PIPELINE.md)
