...
4. **Open-source / non-Gemini providers (UNDER EVALUATION 2026-06).** The earlier "not now" call weighed Flash-Lite vs a tiny cost-floor model (Gemma-3-4B) and found the spread too small to justify a second vendor. The 2026-Q2 wave of capable open-weight models changed the math: GLM-5.2 (~$1.20/$4.10) and DeepSeek V4-Flash (~$0.14/$0.28) are far cheaper than Flash ($1.50/$9.00) / Pro ($2.00/$12.00) at frontier-ish quality. We've built an **evaluation-only** seam + bake-off to test them with hard numbers before migrating anything. Methodology, how-to-run, and results: [`OPEN_MODEL_EVALUATION.md`](./OPEN_MODEL_EVALUATION.md). Nothing has migrated; the production default is still Gemini.
